...
const FAMILY_RULES = [
  ...
  { family: 'Burger Boxes', test: (path) => /burger/i.test(path) },
  ...
  { family: 'Paper Bags', test: (path) => /paper-bag|handle-bag|bags-ireland/i.test(path) },
  { family: 'Business Cards', test: (path) => /business-card/i.test(path) },
  { family: 'Leaflets', test: (path) => /leaflet|services\/leaflets/i.test(path) },
  { family: 'Rubber Stamps', test: (path) => /rubber-stamp|stamps-ireland|(^|\/)stamps?($|\/|-)/i.test(path) },
...
function mapCategoryToFamily(category = '', slug = '', name = '') {
  ...
  if (haystack.includes('business card')) return 'Business Cards';
...
export function parseSearchFromUrl(pageUrl = '') {
  try {
    const url = new URL(pageUrl);
    const search = (url.searchParams.get('q') || url.searchParams.get('query') || '').trim();
    const utmTerm = (url.searchParams.get('utm_term') || '').trim();
    return {
      search_term: search || null,
      utm_term: utmTerm || null,
    };
  } catch {
    return { search_term: null, utm_term: null };
  }
}
