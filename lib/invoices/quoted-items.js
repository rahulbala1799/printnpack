...
export async function repricedQuotedItem(getRows, getRow, quoted, overrides = {}) {
  ...
  if (PLAIN_PRINTED_FAMILIES.has(quoted.family)) {
    plainMaterial = await resolvePlainMaterial(getRows, getRow, quoted.family, params);
  }
...
export function createPendingItem({ label, needs, partial_params = {}, message, subtitle }) {
  return {
    ...
    needs: needs || [],
    partial_params,
    message: message || 'Fill in the details below to calculate cost',
    created_at: new Date().toISOString(),
  };
}

export function upsertPendingItem(items, entry) {
  const key = entry.label?.toLowerCase().trim();
  const idx = items.findIndex(
    (it) => it.status === 'pending' && it.label?.toLowerCase().trim() === key
  );
  if (idx >= 0) {
    const next = [...items];
    next[idx] = { ...next[idx], ...entry, id: next[idx].id };
...
export function setQuotedInvoicePrice(items, { quoted_id, index, unit_price }) {
  return items.map((it) => {
    if (it.id === quoted_id || it.index === index) {
      return { ...it, invoice_unit_price: Number(unit_price) };
    }
    return it;
  });
}

export function createManualQuotedItem({ name, quantity, unit_price, unit_label = 'per unit', unit_cost = null }) {
  ...
  return {
    ...
    unit_label,
    line,
    pricing_params: { name, quantity: qty },
    unit_cost: unit_cost ?? price,
    unit_sell: price,
    invoice_unit_price: price,
    line_total: line.line_total,
    breakdown_structured: null,
    created_at: new Date().toISOString(),
  };
}

/** User is asking to see how a price was built — not a new quote. */
export function userWantsBreakdown(text) {
  return /\b(cost\s+)?break\s*down\b|\bshow\s+(me\s+)?(the\s+)?(cost|price|full)?\s*details?\b|\bbreak\s+it\s+down\b|\bhow\s+(did\s+you|was\s+it)\s+calculat/i.test(
    String(text || '')
  );
}

export function parseQuotedIndexFromText(text) {
  const m = String(text || '').match(/#(\d+)\b|\bitem\s+#?(\d+)\b|\bquote\s+item\s+#?(\d+)\b/i);
  if (!m) return null;
  return parseInt(m[1] || m[2] || m[3], 10);
}

export function getQuotedBreakdowns(items, { index, quoted_id } = {}) {
  let filtered = items;
  if (quoted_id) filtered = filtered.filter((it) => it.id === quoted_id);
  else if (index != null) filtered = filtered.filter((it) => it.index === index);

  return filtered
    .filter((it) => it.breakdown_structured)
    .map((it) => ({
      ...it.breakdown_structured,
      index: it.index,
      id: it.id,
    }));
}
...
export function collectStructuredBreakdownsFromSteps(steps = []) {
  ...
  for (const step of steps) {
    for (const tr of step.toolResults || []) {
      ...
      if (Array.isArray(out?.breakdowns)) {
        for (const b of out.breakdowns) {
          if (b) blocks.push(b);
        }
      }
    }
