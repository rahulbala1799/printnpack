 ...
 */
export async function recalcPrintedLinesForDocument(items, documentType, getRows, getRow, vatRate = 0.23) {
  ...
  for (const line of items) {
    ...
    if (PLAIN_PRINTED_FAMILIES.has(line.pricing_family)) {
      plainMaterial = await resolvePlainMaterial(getRows, getRow, line.pricing_family, params);
    }
