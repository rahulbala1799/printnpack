...
function plainCaseRows(b) {
  const rows = [];
  if (b.plain_product_name) {
    rows.push(row('Product', b.plain_product_name, `#${b.plain_product_id}`));
  }
  if (b.plain_case_label) {
    rows.push(row('Case qty', b.plain_case_label, `${b.plain_units_per_case} units/case`));
  }
  if (b.plain_price_per_case_ex != null) {
    rows.push(row('Case price (ex-VAT)', eur(b.plain_price_per_case_ex), b.plain_tier || 'tier 1'));
  }
  if (b.plain_unit_cost_ex != null) {
    rows.push(row('Plain unit (ex-VAT)', eur(b.plain_unit_cost_ex), 'per sellable unit'));
  }
  return rows;
}
...
export function structuredBreakdownForFamily(family, result, params = {}) {
  ...
  if (family === 'pizza_box_printed') {
    base.sections = [
      section('Plain box (cost list)', [
        ...plainCaseRows(b),
        row('CMYK ink / box', eur(b.inkPerBoxEx)),
    ...
    base.summary = [
      sumRow('Plain unit cost', eur(b.plain_unit_cost_ex)),
      sumRow('Cost / box (incl print)', eur(b.unitCost)),
  ...
  if (
    ...
    family === 'corrugated_meal_box_printed' ||
    ...
    base.sections = [
      section('Plain packaging', [
        ...plainCaseRows(b),
        row('Ink / unit', eur(b.inkPerUnitEx)),
    ...
    base.summary = [
      sumRow('Plain unit cost', eur(b.plain_unit_cost_ex)),
      sumRow('Cost / unit (incl print)', eur(b.unitCost)),
      sumRow('Pricing', b.pricing_mode || '30% markup'),
