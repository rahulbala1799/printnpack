...
import { hasSearchConsoleData, loadSearchConsoleData, resolvePeriodDir } from './seo/search-console';
  ...
  return `
    COALESCE(
      ...
      CASE
        ...
        WHEN ${pagePathSql()} ~* 'rubber-stamp|(^|/)stamps?(/|$|-)' THEN 'Rubber Stamps'
...
function gscByPath() {
  try {
    const dir = resolvePeriodDir('7d');
    if (!hasSearchConsoleData(dir)) return new Map();
    const data = loadSearchConsoleData(dir);
    const map = new Map();
