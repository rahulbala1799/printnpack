...
import {
  ...
  GLOVES_HUB_PATH,
  NITRILE_GLOVES_CONFIG,
  NITRILE_GLOVES_HUB_PATH,
  getGloveDisplayName,
  isNitrileGlove,
} from '../data/gloves-seo';

const PAGE_URL = `${SITE_URL}${NITRILE_GLOVES_HUB_PATH}`;
const HERO_IMAGE = '/images/plain-packaging/170054.webp';
const DISCOUNT = 0.95;

const nitrileGloves = PLAIN_PRODUCTS.filter(
  (p) => p.category === GLOVES_CATEGORY && isNitrileGlove(p)
...
const breadcrumbLd = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 3, name: 'Nitrile Gloves Ireland', item: PAGE_URL },
  ],
};
...
const webPageLd = {
  '@context': 'https://schema.org',
  '@type': 'WebPage',
  name: NITRILE_GLOVES_CONFIG.metaTitle,
  description: NITRILE_GLOVES_CONFIG.metaDescription,
  url: PAGE_URL,
  inLanguage: 'en-IE',
  isPartOf: { '@type': 'WebSite', name: 'PrintNPack Ireland', url: SITE_URL },
  about: { '@type': 'Thing', name: 'Nitrile disposable gloves Ireland wholesale' },
  dateModified: '2026-07-05',
};

export default function NitrileGlovesIreland() {
  const { metaTitle, metaDescription, keywords, h1, intro } = NITRILE_GLOVES_CONFIG;
  const blueNitrile = nitrileGloves.filter((p) => p.name.toLowerCase().includes('blue'));
  const blackNitrile = nitrileGloves.filter((p) => p.name.toLowerCase().includes('black'));
    ...
    <Layout>
      ...
      <nav className="bg-gray-50 border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <ol className="flex items-center gap-2 text-sm text-gray-500">
            ...
            <li>/</li>
            <li className="text-gray-800 font-medium">Nitrile Gloves</li>
          </ol>
        </div>
      </nav>
          ...
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            <div>
              <p className="text-sm font-semibold text-sky-700 uppercase tracking-wider mb-3">
                Powder-free · blue &amp; black · S to XL
              </p>
            ...
            <div className="relative aspect-square rounded-2xl overflow-hidden bg-stone-50 border border-gray-200">
              <Image
                src={HERO_IMAGE}
                alt="Blue nitrile gloves Ireland wholesale — powder-free disposable catering gloves"
                fill
      ...
      <section className="py-12 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">Blue nitrile gloves</h2>
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-6">Black nitrile gloves</h2>
          ...
          <p className="text-sm text-gray-600 mt-8 max-w-2xl">
            Need economical vinyl gloves? Browse our{' '}
            <Link href="/vinyl-gloves-ireland" className="text-sky-700 hover:underline font-medium">
