...
import { NAPKIN_FAQ_CATEGORIES, NAPKIN_FAQS } from '../data/napkin-faq';

const PAGE_URL = `${SITE_URL}/napkin-faq-ireland`;
const HERO_IMAGE = '/images/hero/napkin.svg';

const faqLd = {
  '@context': 'https://schema.org',
  '@type': 'FAQPage',
  mainEntity: NAPKIN_FAQS.map(({ q, a }) => ({
    '@type': 'Question',
    name: q,
...
const breadcrumbLd = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 3, name: 'Napkin FAQ', item: PAGE_URL },
  ],
};
...
export default function NapkinFaqIreland() {
  const [query, setQuery] = useState('');
  const [activeCategory, setActiveCategory] = useState('all');

  const title = 'Napkin FAQ Ireland | Printed, Branded & Linen-Feel Napkins';
  const description =
    'Instant answers to napkin printing questions — costs, sizes, cocktail vs dinner napkins, linen-feel vs paper, wedding napkins, and delivery across Ireland.';

  const filteredFaqs = useMemo(() => {
    const normalizedQuery = normalizeSearch(query);
    return NAPKIN_FAQS.filter((faq) => {
  ...
  const groupedFaqs = useMemo(() => {
    ...
    return NAPKIN_FAQ_CATEGORIES.map((category) => ({
      ...category,
  ...
  return (
    <Layout>
      <Head>
        ...
        <meta name="keywords" content="napkin faq, printed napkins ireland, personalised napkins, cocktail napkins, linen feel napkins, napkin printing, wedding napkins, branded napkins ireland" />
      ...
      <section className="bg-white border-b border-gray-100">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 lg:py-14">
          <p className="text-sm font-semibold text-amber-600 uppercase tracking-wider mb-3">Detailed FAQ</p>
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 leading-tight mb-4">Napkin FAQ — Ireland</h1>
          ...
          <div className="relative mb-6">
            <label htmlFor="napkin-faq-search" className="sr-only">Search napkin questions</label>
            <input
              id="napkin-faq-search"
              type="search"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Search questions… e.g. cocktail napkins, wedding, linen feel, cost"
              className="w-full rounded-xl border border-gray-300 bg-white px-4 py-3.5 pr-12 text-gray-900 placeholder:text-gray-400 focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-100"
            />
          </div>

          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => setActiveCategory('all')} className={`rounded-full px-4 py-2 text-sm font-medium transition-colors ${activeCategory === 'all' ? 'bg-amber-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>All questions</button>
            {NAPKIN_FAQ_CATEGORIES.map((category) => (
              <button key={category.id} type="button" onClick={() => setActiveCategory(category.id)} className={`rounded-full px-4 py-2 text-sm font-medium transition-colors ${activeCategory === category.id ? 'bg-amber-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>{category.label}</button>
            ))}
          </div>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          {filteredFaqs.length === 0 ? (
            <div className="rounded-2xl border border-gray-200 bg-white p-8 text-center">
              ...
              <Link href="/products/printed-napkins" className="inline-flex items-center bg-amber-600 text-white font-semibold px-6 py-3 rounded-xl hover:bg-amber-700 transition-colors mt-4">Ask us directly</Link>
            </div>
          ) : (
                    ...
                    {group.faqs.map((faq, index) => (
                      <details key={faq.id} id={faq.id} className="group rounded-xl border border-gray-200 bg-white open:shadow-sm" defaultOpen={!query.trim() && activeCategory === 'all' && index === 0 && group.id === 'pricing'}>
                        ...
                        <div className="px-5 pb-5 border-t border-gray-100">
                          ...
                          {faq.link && (
                            <Link href={faq.link.href} className="inline-flex items-center gap-1 text-sm font-semibold text-amber-600 hover:text-amber-700 mt-4">
      ...
      <section className="bg-white border-y border-gray-100">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 lg:py-12">
          ...
          <p className="text-gray-600 mb-6">Tell us your napkin size, quantity, and logo — we help with artwork and pricing.</p>
